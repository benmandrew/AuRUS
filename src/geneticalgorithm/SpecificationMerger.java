...
public class SpecificationMerger {
    ...
    public static List<Tlsf> merge(Tlsf spec0, Tlsf spec1, SPEC_STATUS status0, SPEC_STATUS status1, int level) {
        ...
        if (level == 0) {
            ...
            Tlsf new_spec = TlsfUtils.fromSpec(spec0);
            // set assume
            turn = Settings.RANDOM_GENERATOR.nextInt(2);
            if (turn == 0)
                new_spec = TlsfUtils.change_assume(new_spec, spec0.assume());
            else
                new_spec = TlsfUtils.change_assume(new_spec, spec1.assume());

            // set guarantees
            turn = Settings.RANDOM_GENERATOR.nextInt(2);
            if (turn == 0)
                new_spec = TlsfUtils.change_guarantees(new_spec, spec0.guarantee());
            else
                new_spec = TlsfUtils.change_guarantees(new_spec, spec1.guarantee());
            ...
            List<LabelledFormula> assumesspec0 = FormulaUtils.splitConjunction(assumspec0);

            LabelledFormula assumspec1 = LabelledFormula.of(spec1.assume(), spec1.variables());
            List<LabelledFormula> assumesspec1 = FormulaUtils.splitConjunction(assumspec1);

            List<Formula> f = getRandomFormulas(assumesspec0);
            f.addAll(getRandomFormulas(assumesspec1));
            new_spec = TlsfUtils.change_assume(new_spec, f);

            // set guarantee
            new_spec = TlsfUtils.change_guarantees(new_spec, mergeGuarantess(spec0.guarantee(), spec1.guarantee()));
